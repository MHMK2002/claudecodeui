...
const selections: Record<LLMProvider, ResolvedProviderSelection> = {
  claude: { provider: 'claude', providerProfileId: 11, model: 'claude-model' },
  codex: { provider: 'codex', providerProfileId: 12, model: 'codex-model' },
  cursor: { provider: 'cursor', providerProfileId: null, model: 'cursor-model' },
  opencode: { provider: 'opencode', providerProfileId: null, model: 'opencode-model' },
};
...
test('validates and forwards the exact four-provider selection into an isolated read-only run', async () => {
  const validations: ResolvedProviderSelection[] = [];
  const runs: Array<{
